...
namespace layer_3.s
{
    class db_factory : s_db_factory
    {
        ...
        {
            return new db<T>(a.s_db.GetDatabase(xid).GetCollection<T>("x_" + typeof(T).Name));
        }
        public s_db<T> a_user<T>(string userid) where T : m_id
        {
            return new db<T>(a.s_db.GetDatabase(xid).GetCollection<T>("u_" + userid + "_" + typeof(T).Name));
        }
    }
}