...
namespace z_x_user.z
{
    class upsert_contact : y_upsert_contact
    {
        protected async override void implement()
        {
            ...
            {
                reply(new o() { z_error = error.invalid_parametrs });
                return;
            }
            if (await more.user(a_partner_id))
            {
                reply(new o() { z_error = error.invalid_partner_id });
                return;
            }
            await mine();
            await partner();
        ...
        private async Task mine()
        {
            ...
            contact.mine.Remove(a_partner_id);
            if (a_contact)
                contact.mine.Add(a_partner_id);
        ...
        private async Task partner()
        {
            ...
            contact.partners.Remove(a_partner_id);
            if (a_contact)
                contact.partners.Add(a_partner_id);
            await db.upsert(contact);
        }
    }
}
